import{_ as s,c as a,o as n,a4 as p}from"./chunks/framework.DvHWMJgD.js";const e="/images/node11-1.png",i="/images/node11-2.png",l="/images/node11-3.png",t="/images/node11-4.png",c="/images/node11-5.png",h="/images/node11-6.png",o="/images/node11-7.png",r="/images/node11-8.png",d="/images/node11-9.png",_=JSON.parse('{"title":"❤11-登录人token信息接口","description":"","frontmatter":{},"headers":[],"relativePath":"src/Node/Node09-登录人token接口.md","filePath":"src/Node/Node09-登录人token接口.md"}'),k={name:"src/Node/Node09-登录人token接口.md"},g=p(`<h1 id="❤11-登录人token信息接口" tabindex="-1">❤11-登录人token信息接口 <a class="header-anchor" href="#❤11-登录人token信息接口" aria-label="Permalink to &quot;❤11-登录人token信息接口&quot;">​</a></h1><p>上一章我们已经从登录部分拿到了用户的登录jwt返回的token信息，接下来我们就通过token来换取用户信息 这里我们可以将其理解为一种加密以及解密的思想来思考这个jwt和token的关系，token就是一个加密的字符串，而jwt就是那把钥匙。</p><h2 id="_1、解析-jwt字符串-还原为json对象" tabindex="-1">1、解析 JWT字符串 还原为JSON对象 <a class="header-anchor" href="#_1、解析-jwt字符串-还原为json对象" aria-label="Permalink to &quot;1、解析 JWT字符串 还原为JSON对象&quot;">​</a></h2><p>客户端每次在访问那些有权限接口的时候，都需要主动通过请求头中的 <code>**Authorization**</code> 字段，将 Token 字符串发送到服务器进行身份认证。 此时，服务器可以通过 <strong>express-jwt</strong>这个中间件，自动将客户端发送过来的 Token 解析还原成 JSON 对象： token解析如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>app.use(</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">     ]</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">  })</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">);</span></span></code></pre></div>`,59),u=[g];function E(m,y,b,v,C,F){return n(),a("div",null,u)}const f=s(k,[["render",E]]);export{_ as __pageData,f as default};
